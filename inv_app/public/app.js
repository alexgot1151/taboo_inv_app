(() => {
  ...
  function updateLowBanner() {
    const lowItems = state.alcohols.filter((a) => a.quantity <= 150);
    if (!lowItems.length) {
      lowAlert.classList.add('hidden');
      return;
    }
    lowAlert.classList.remove('hidden');
    const names = lowItems.map((a) => a.name).join(', ');
    lowAlert.textContent = `Low stock: ${names} at or below 150ml`;
  }
    ...
    sorted.forEach((item) => {
      ...
      card.innerHTML = `
        <div class="item-header">
          <div>
            <p class="item-name">${item.name}</p>
            <p class="item-qty">${item.quantity} in stock</p>
          </div>
          <span class="pill ${item.quantity ? 'ok' : 'warn'}">${item.quantity ? 'Ready' : 'Out'}</span>
        </div>
        <div class="item-actions">
          <button class="primary-btn" data-action="serve-shisha" data-name="${item.name}">Serve 1</button>
          <button class="secondary-btn" data-action="restock-shisha" data-name="${item.name}">+1</button>
  ...
  async function addShisha(name, quantity) {
  ...
  addShishaForm.addEventListener('submit', async (e) => {
    ...
    const quantity = Number(addShishaForm.quantity.value || 10);
    if (!name) return;
    try {
      await addShisha(name, quantity);
      addShishaForm.reset();
